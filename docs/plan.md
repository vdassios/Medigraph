...
ADR-0015 withdraws the proxy and leaves the data rule standing alone. **ADR-0020**
records that an identifier candidate says whether the container labelled it, and that
`beginReview` answers those for the user. ADR-0010 records D13. **ADR-0011** records why no vision-language
...
interface IdentifierCandidate {
  ...
  knownPosition: boolean; // the container's own labelled field (ADR-0020)
  sourceRef?: SourceRef;
}
...
export type RouteFailure =
  ...
  | {
      ...
      fileName: string;
      code:
        'unsupported-type' | 'file-too-large' | 'decode-failed' | 'not-ahfy-document';
    };
...
// A batch-scoped failure ends the run and keeps what already succeeded: work
// the user watched finish is theirs, and `too-many-files` reports an empty
// `results` only because it is decided before anything is read. The type check
// precedes the size check — a photograph is the wrong kind of thing whatever it
// weighs — and its MIME half is asked of the adapter's own `supports`, so there
// is one answer to "is this mine". `onProgress` fires twice per source, at
// nought pages and at the page count decode returned; finer granularity needs
// `extractPdfText` to take a callback, which is a signature change.
// `ROUTE_LIMITS` is exported because `FileDrop` states the caps before a file is
// chosen: the attach screen prints the numbers this module applies rather than
// numbers of its own, and it prints them only — the check stays here, so the
// product never holds two answers to the same question.
...
export type EvidenceLookup =
  ...
  | { kind: 'unknown-source' }
  | { kind: 'unknown-page' };
// `onInspectSource` returns the lookup rather than performing it, which is what
// Task 4.0 built: the island owns the map and answers a question about it, and
// the crop belongs beside the row that asked (Task 4.2a). A `void` callback
// would put "which row is being inspected" in the parent, which owns the
// transaction and not the table's layout.
...
`preact@10`, `zod@4.5.4`, `pdfjs-dist@5`, `idb@8`, `vitest`, `@playwright/test`. Task 3.1 adds
the one PDF stack, `pdfjs-dist`; the OCR selection task that once owned that line died
with E1 (ADR-0013). No OCR runtime, model or dictionary ships. Test-only packages are
not browser runtime bytes and so are outside D1: Task 3.6 adds `fake-indexeddb` as a
devDependency, because `idb` needs the whole IndexedDB surface and Node has none of
it. Task 4.1 adds `jsdom` on the same footing, because a Preact component needs a
document and Node has none; `happy-dom` was tried first and rejected —
it does not implement the `ondrop`/`ondragover` IDL attributes, so Preact registers the
drop listener under a capitalised event name, and a drag-and-drop test passing there
would be testing something no browser does.
...
**It detects; `beginReview` resolves the labelled ones.** A candidate says whether the
container's own fixed label named it (`knownPosition`, ADR-0020), and `beginReview`
pre-resolves those as `redacted` — through `resolveIdentifier`, so the text leaves the
drafts' free text exactly as it would had the user answered. Those resolutions stay the
user's to change, though the text does not come back, which is true of every redaction.
A shape scanned out of a line is never pre-resolved: only the reader knows whether an
eleven-digit number in prose is an ΑΜΚΑ. Putting approved unknown marker labels in the
scrub surface is review's duty (Task 4.2) with `assertProfileSafe` as the final gate —
this module never sees a marker label. Candidates are transient and never persisted.
...
**Correcting a value, and answering D8.** `editRowMeasurement` rewrites the seven
fields that say what was measured and nothing else: the row keeps its id, marker key,
printed label, `sourceRef` and `flags`, because renaming the quantity is
`reassignMarker`'s question and a flag describes the parse that happened rather than
the value now standing. It drops the fields the new status cannot carry — D15 makes a
Measurement numeric **or** categorical, and a screen offering both sets of inputs will
eventually hand over both. A `missing` row keeps its unit and reference range, since
what the laboratory printed beside a value nobody could read is still what it printed;
a categorical row keeps neither, because the persisted schema forbids both. No conflict rebuild: a marker key cannot
move through it. `confirmSamePerson` records the D8 answer, `null` withdrawing it; the
question is put to the user and never to the document, whose ΑΜΚΑ is redacted at the D7
gate and never compared. Both were added by Task 4.2, which needed a path to the two
gates the screen must present and found none.
...
| 0.3  | **Synthetic/redacted seed fixtures.** Derive from the ΑΗΦΥ documents under `corpus/greek-labs/gov/` — every other `corpus/` subdirectory is deprecated pre-ADR-0013 material and is not a fixture source. One ΑΗΦΥ document consolidates every department, so there is no per-panel split: commit `fixtures/seed/ahfy-full.{pdf,textitems.json,expected.json}` (all panels, Greek-name labels), `fixtures/seed/ahfy-minimal.{pdf,textitems.json,expected.json}` (CBC only, bare-abbreviation labels, a second laboratory) and `fixtures/seed/not-ahfy.pdf` (a loose laboratory PDF Pass V must reject). Preserve the real structural chrome Pass V validates — title, the twelve metadata labels, section titles, the five-column header and the column x-bands — with synthetic identity and values. Commit fragmented (token-level) and whole-line observation variants. Never commit a root PDF.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | No real identity substring or document metadata survives. Expected output includes dates, comparators, missing status, retained ranges and SourceRefs. Human/capable-model task.                                                                                                                                                                                                                                                            |
| 0.4  | **Static security foundation—not the final privacy E2E.** Self-hosting every browser byte under `public/` is the v1 default, not an obligation (ADR-0015). There is **no** generated content-hash asset manifest and **no** generated CSP hash list: the policy is a committed static string. (The service worker's precache list is a different artifact and _is_ generated — ADR-0017 — because nothing in the CSP depends on it.) Commit `_headers` with CSP `default-src 'none'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'; style-src 'self'; style-src-elem 'self' 'unsafe-inline'; style-src-attr 'unsafe-inline'; connect-src 'self'; worker-src 'self' blob:; img-src 'self' blob: data:; font-src 'self'; manifest-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-src 'none'; frame-ancestors 'none'`. `connect-src` names the origins the app actually uses — `'self'` alone while every asset is self-hosted — and is the load-bearing directive: it is XSS containment for the plaintext Profile, not egress policy (ADR-0015), and adding an origin edits this one line and needs no decision record. `script-src` carries `'unsafe-inline'` because Astro delivers a client island through two inline `<script>` blocks; that is how the framework works, and the alternatives all buy a generated hash list or a hand-rolled mount ([ADR-0016](adr/0016-inline-scripts-and-the-withdrawn-isolation-headers.md)). `'wasm-unsafe-eval'` covers pdf.js image-codec WebAssembly and permits no inline or `eval`'d JavaScript. Set `build.inlineStylesheets: 'never'` so component `<style>` blocks always emit as external `'self'` stylesheets and no style hash is ever required; `style-src-attr 'unsafe-inline'` is the deliberate, scoped exception recorded in [ADR-0008](adr/0008-csp-style-attribute-amendment.md), and `style-src-elem 'unsafe-inline'` the one recorded in [ADR-0019](adr/0019-astro-island-styles-are-permitted.md): Astro emits its island shim as an inline `<style>` element that `inlineStylesheets: 'never'` does not govern. `style-src` itself is **not** relaxed, and because the engines that ignore the Level 3 split fall back to it, `display: contents` on an island wrapper must never be load-bearing. Record which target browsers honour `style-src-attr` and which fall back to `style-src`. Add COOP `same-origin`, CORP `same-origin`, `Referrer-Policy: no-referrer`, HSTS, `X-Content-Type-Options: nosniff`, and a Permissions Policy disabling camera, microphone, geolocation, payment and USB — with E1 deleted the product has no camera path, so `camera=()` is the correct value (ADR-0013). Do **not** set COEP `require-corp`: its only beneficiary was Task 3.3's WASM threads, which ADR-0013 deleted (ADR-0016). | A unit test asserts the committed `_headers` byte-for-byte, including that `'unsafe-inline'` appears on `script-src`, `style-src-elem` and `style-src-attr` and nowhere else; the built app boots under the delivered headers with no CSP violation, asserted in Playwright on a page **load** rather than only on interaction (ADR-0019 records why the original assertion could not see one). Full workflow egress testing waits for 5.2. |
| 0.5a | **ΑΗΦΥ corpus.** Hand-checked TextItem + expected fixtures from every ΑΗΦΥ document supplied, one directory per issuing laboratory, each expected row naming the marker it measures. Cover the observed content dialects: Greek-name labels, bare Latin-code labels, comma and period decimals, unit-inside-range, `(LABEL …)` structural rows and the qualitative urine panel. Redacted TextItems only; never commit a source document. Hold one entire issuing laboratory blind.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Measures parser/registry behavior without pretending to test OCR. Human/capable-model task.                                                                                                                                                                                                                                                                                                                                                 |
...
- **An expected row states its `markerKey`.** Identity is part of what a reader states
  about a printed row, and the score pairs on it, so deriving it instead leaves only the
  parser being scored to supply it. A canonical key must name a marker the registry
  actually holds; anything else is the `x:` key, which is the positive claim that the
  registry has no entry for this row. Two rows in one document never share a key.
...
| 2.6  | `profile.ts` — one Report per document, explicit report construction, mandatory conflict resolution, same-person append, and id-based profile merge                                                                                                                                                                                 | 2.5a, 1.6b-core, 1.9      |
...
(`Αρνητικό ίχνη ή`) rather than the printed line order.

`readAnchor` is now given the columns Pass V bound, and consults them for exactly two
judgements the anchor's own box cannot make: whether the text after the matched span is
another cell or the rest of the label — an item ending inside the label column holds no
cells, which is what `Λευκά αιμοσφαίρια (WBC) (WBC)` needs — and whether a wide
horizontal gap is a search wandering off or the distance from a short label to the value
column, which is what `RDW` and a wrapped `(MCHC)` fall into. They locate no value and
are not a second algorithm; a caller with no validated template passes `null` and gets
the untemplated outward search unchanged. Re-ordering the tokens inside one wrapped
cell, which is what `Ουροχολινογόνο` still needs, remains undone.
...
| 3.6 | `storage.ts` — plaintext IndexedDB `saveProfile`, `loadProfile`, atomic `replaceProfile`, `clearAll` and first-save `navigator.storage.persist()` result. Database `medigraph`, one store `profile`, one key `current`. A stored value that no longer validates **throws** rather than reading as empty storage, which would invite the next save to overwrite a history nobody knew was there.                                                        | 0.1, 0.2          | Store exactly one Profile and no drafts/evidence, and nothing else. `clearAll` removes the database and every Medigraph Cache Storage entry; app state separately disposes live object URLs/bitmaps.                                                                                                                                                                 |
| 3.7 | `sw.js` — versioned cache of an explicit, committed app asset list only; cache-first exact-path lookup after first visit. The list is generated by `scripts/app-assets.mjs` and the cache name derived from it (ADR-0017); the worker itself is hand-written. **Every cache name begins `medigraph-`**, because `clearAll` (3.6) deletes Medigraph caches by that prefix and must reach a name written by a build it has never heard of                | 0.4               | Never cache navigations with user data, blob/data URLs, source files or arbitrary request URLs. No push, background/periodic sync or dynamic `importScripts`. Scope to the app; upgrades remove old asset caches. E1 remains usable offline after assets were first fetched.                                                                                         |
...
The service worker's explicit source of truth is `public/app-assets.json`. It lists
exact same-origin URL paths for the app shell, hashed JS/CSS output and the pdf.js
worker bytes. **`scripts/app-assets.mjs` generates it from the production build**, as
part of `pnpm build` ([ADR-0017](adr/0017-the-service-worker-asset-list-is-generated.md)):
the list is mechanical — a reviewer cannot approve or reject a content hash on its
merits — while the caching policy is hand-written and stays that way. The file remains
committed, so an asset change is still visible in a diff, and the generator writes
identical bytes when nothing moved. Tests fail when a listed path is absent from
`dist/`, when a required runtime path is missing, when the committed list disagrees with
its own revision, or when an unlisted request is added to the cache.

**The cache name is derived, not declared** — `medigraph-assets-<revision>`, where the
revision hashes the sorted list. It changes exactly when the cached set changes, which
is the condition under which the previous cache must be evicted: the shell paths `/` and
`/app/` are not content-hashed, so a cache kept across builds would serve yesterday's
HTML forever. Deriving it removes the chance of anyone forgetting to bump it.

**The worker answers only from a list it can read synchronously.** A `fetch` handler
must decide whether to call `respondWith` before it can await anything, so a worker that
loaded its list on demand would have to intercept every request in order to discover it
wanted none of them. The list is read once at script evaluation and held; in the window
before it arrives — a restarted worker re-evaluates without installing again — the
worker declines every request and the browser behaves exactly as if nothing were
installed. Unlisted requests get no `respondWith` at all, so there is no heuristic to get
wrong and nothing a request can say that puts it in the cache.
...
| 4.1  | `FileDrop` — drag/drop and a file picker that works on a phone; **no camera**: ADR-0013 deleted every recognition path, so the input carries no `capture` attribute and Task 0.4's Permissions Policy keeps `camera=()`. Surface the 3.4 limits from the router's own `ROUTE_LIMITS` rather than restating them, and leave the enforcing to `routeFiles`; show per-file/page progress; render each typed `RouteFailure`. `too-many-files`, `too-many-pages` and `cancelled` are batch errors; `unsupported-type`, `file-too-large`, `decode-failed` and `not-ahfy-document` identify a source. Source failures do not discard successful siblings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 4.2  | `ReviewTable` — one session, one proposed Report per document. Preserve an ephemeral `ParsedRow.id`→edited draft mapping until Report construction; it never enters Profile. Every Report requires date confirmation; rows support edit/delete and searchable canonical marker reassignment or approved unknown. Reassignment recomputes duplicate conflicts. Conflicts require choose/edit; source page/crop opens beside a row. Low/flagged rows sort first. Identifier panel requires Redact (mask evidence and remove the substring from every derived field), Delete affected row, or False positive for every candidate; unknown labels always appear. Confirm remains disabled until all gates pass, then writes all Reports atomically and disposes evidence. Three notes from the build: a conflict is resolved by choosing a candidate, and correction happens by editing a candidate row and then choosing it, so no `Measurement` is ever constructed outside the domain — `Conflict.resolution.kind: 'edited'` stays in the contract for a replacement that is no candidate row. The screen also refuses Confirm while an edit is open and unsaved, since the typed value is not in the session and Confirm would write the one it replaces; a screen may narrow what `canConfirm` permits and may never widen it. And the evidence beside a row is the source text the extractor read on that page, named by document and page: no rasteriser ships, because `pdfRaster.ts` went with E1 (ADR-0013), so "crop" means the source line until one does.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 4.2a | **Review-friction reduction.** A presentation layer over `ReviewTable`: it adds no domain function, weakens no gate and changes no `ReviewSession` shape. (a) **Confidence triage** — a row with `confidence: 'high'` and an empty `flags` array renders collapsed inside a pre-accepted group showing a count and a “review these anyway” disclosure; every other row renders expanded, flagged first, exactly as in 4.2. (b) **Batch confirm** — the primary action accepts all pre-accepted rows at once, with per-row exception still available. It stays disabled until every D6 and D7 gate passes, unchanged from 4.2: triage must never collapse an unresolved identifier candidate, an unconfirmed date, an unresolved conflict or an unapproved unknown marker, and those render expanded regardless of confidence. (c) **Inline correction** — editing a row opens its `SourceRef` crop beside the field rather than in a separate view, reusing the 4.0 `onInspectSource` callback. State in the issue that registry coverage (D5a) is the largest single lever on how many rows need touching, and that this task reduces review effort without reducing review authority. **Pass/fail (Playwright):** a fixture whose rows are all high-confidence and unflagged **reaches Confirm in one action** — every row collapses into the pre-accepted count, every identifier sits in a labelled position `beginReview` has already answered (ADR-0020), and the one action left is the date D6 insists on. The second fixture keeps every flagged, hedged, unapproved or contested row expanded — asserted as the partition it is, `expanded + pre-accepted = rows`, with nothing flagged or below high confidence inside the disclosure — and holds Confirm disabled until each gate is answered. The seed corpus prints `unrecognised-unit` and `unparsed-range` rather than `implausible-value`; fixtures are derived from supplied ΑΗΦΥ documents and are never authored to order. |
...
**Copy in Wave 4 is written for Task 4.6's translation.** Tasks 4.1–4.5 write their
user-facing strings inline in Greek, because the `el`/`en` toggle and its copy deck do
not exist until 4.6. Each component therefore keeps every string it renders in one
named block, and every string is a whole sentence: a message assembled at render time
from a shared prefix and two endings cannot be translated, since word order is the
first thing another language changes. Lifting a component's copy in 4.6 is then a move,
not a hunt through its JSX.
...
| #   | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 5.1 | Happy-path Playwright E2E: attach one synthetic ΑΗΦΥ document → confirm its collection date → resolve review → one Report with 25+ markers → panel → ferritin trend → plaintext export → clear/reload → import preview → identical Profile                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
on 4.2, 4.2a, 4.3, 4.4 and 4.6. Task 5.6 depends on 2.6,
...
parser-corpus tests consume committed redacted TextItems. Component tests under `src/ui/`
run in a DOM: the default Vitest environment stays `node`, since `domain/` has no DOM,
and each `*.test.tsx` opts into `jsdom` with a `// @vitest-environment jsdom` docblock.
Components are driven through Preact's own `render` and `preact/test-utils` `act`; no
component-testing library is installed.
